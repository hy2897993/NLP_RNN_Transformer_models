...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.linear1 = nn.Linear(self.input_size+self.hidden_size, self.output_size)
        self.log_softmax = nn.LogSoftmax()

        # initialize block that creates output
        
        self.linear2 = nn.Linear(self.input_size+self.hidden_size, self.hidden_size)
        self.tanh = nn.Tanh()
    ...
    def forward(self, input, hidden):
        ...
        #############################################################################
        concat = torch.concatenate((input, hidden), axis = 1)
        out_L1 =  self.linear1(concat)
        out_L2 =  self.linear2(concat)
        
        output = self.log_softmax(out_L1)
        hidden = self.tanh(out_L2)
