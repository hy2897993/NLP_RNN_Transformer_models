...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        encoder_output, first_hidden_state = self.encoder(source)
        
        first_input = source[:, 0]
        outputs[:, 0, :], first_hidden_state = self.decoder(first_input.unsqueeze(1), first_hidden_state, encoder_outputs=encoder_output, attention=self.attention)

        input = outputs[:,0,:].argmax(1)
        # first_input = source[:, 0]
        for i in range(1, out_seq_len):
            outputs[:,i,:], first_hidden_state = self.decoder(input.unsqueeze(1), first_hidden_state, encoder_outputs=encoder_output, attention=self.attention)
            input = outputs[:,1,:].argmax(1)
